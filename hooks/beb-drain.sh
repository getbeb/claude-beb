...
# The pin: missing, or relative and therefore refused by beb. Both are
# silent failures here, so both are decided in one place.
. "$(dirname "$0")/beb-pin.sh"
beb_pin "$input" "${1:-}"
...
# The line names the pin it speaks for. A session is whichever role its
